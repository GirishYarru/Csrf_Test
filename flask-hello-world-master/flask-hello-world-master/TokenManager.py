...
class SessionSecureForm():
    ...
    def validate_csrf_token(self, field):
       # if not field.data or '##' not in field.data:
        ...
        expires, hmac_csrf = field.split('##')
        print('=====expires===== ', expires)
        print('=====hmac_csrf===== ', hmac_csrf)
        print('=====session[csrf]===== ', session['csrf'])
        #check_val = (session['csrf'] + expires).encode('utf8')
        check_val =  '%s%s' % (session['csrf'], expires)
        print('=====check_val===== ', check_val)


        hmac_compare = hmac.new(self.SECRET_KEY, check_val.encode('utf8'), digestmod=sha1)
        print('=====hmac_compare===== ', hmac_compare.hexdigest())
